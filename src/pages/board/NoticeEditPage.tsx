...
export function NoticeEditPage() {
  ...
  useEffect(() => {
    if (id && user) {
      fetchNoticeById(id, user.id).finally(() => setIsFormLoading(false));
    }
  ...
  if (!isFormLoading && (!notice || !canEdit)) {
    navigate('/notices');
    return null;
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Navigate to the notice detail page
      navigate(`/notices/${id}`);
    } catch (error) {
  ...
  const handleCancel = () => {
    if (
      window.confirm('수정 중인 내용이 있습니다. 정말 취소하시겠습니까?')
    ) {
      navigate(`/notices/${id}`);
    }
  };
